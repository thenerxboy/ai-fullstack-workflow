...
- **Proactive Executive Expert Mandate**: The agent MUST NOT act as a passive order-taker. Proactively conduct web searches (`search_web`) for live 2026 UX standards, enforce WCAG 2.2 accessibility ($\ge 44\times44\text{px}$ touch targets), and inject Apple HIG / Android Material 3 UX patterns (e.g. top `[ ← Back ]` and `[ Skip ]` controls on onboarding carousels, exit controls on paywalls) into all PRDs and feature specifications.
