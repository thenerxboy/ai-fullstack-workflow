...
- **8-Step Vibe Engineering Loop**: Read rules & tech stack $\rightarrow$ Accept focused prompt $\rightarrow$ Write implementation plan to `prompts/<task-name>.md` $\rightarrow$ Human review $\rightarrow$ Human approval ("yes") $\rightarrow$ Implement code on branch $\rightarrow$ Run typecheck (`tsc`) & lint checks $\rightarrow$ CodeRabbit PR & Main Sync.
