...
15. **Proactive Executive Expert & Usability Mandate**: DO NOT act as a passive order-taker. The agent MUST proactively inject Apple HIG / Android Material 3 UX controls (top `[ ← Back ]` and `[ Skip ]` controls on onboarding carousels, explicit exit controls `[ × ]` on paywall modals, WCAG 2.2 touch targets $\ge 44\times44\text{px}$) and perform web search audits (`search_web`) for 2026 UX standards before outputting prompt blueprints.
